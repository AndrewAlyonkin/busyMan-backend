...
BEGIN
    ...
    if (ifnull(NEW.category_id, 0) > 0 && ifnull(NEW.completed, 0) = 1) then
        update category set completed_count = (ifnull(completed_count, 0) + 1) where id = NEW.category_id and user_id = NEW.user_id;
    end if;
...
BEGIN
    ...
    if (ifnull(old.category_id, 0) > 0 && ifnull(old.completed, 0) = 1) then
        update category set completed_count = (ifnull(completed_count, 0) - 1) where id = old.category_id  and user_id = old.user_id;
    end if;
    ...
    if (ifnull(old.category_id, 0) > 0 && ifnull(old.completed, 0) = 0) then
        update category set uncompleted_count = (ifnull(uncompleted_count, 0) - 1) where id = old.category_id  and user_id = old.user_id;
    end if;
...
BEGIN
    ...
    IF (ifnull(old.completed, 0) = ifnull(new.completed, 0) && new.completed = 1 &&
        ...
        update category set completed_count = (ifnull(completed_count, 0) + 1) where id = new.category_id  and user_id = old.user_id;
    END IF;
    ...
    IF (ifnull(old.completed, 0) = ifnull(new.completed, 0) && new.completed = 0 &&
        ...
        update category set uncompleted_count = (ifnull(uncompleted_count, 0) + 1) where id = new.category_id and user_id = old.user_id;
    END IF;
